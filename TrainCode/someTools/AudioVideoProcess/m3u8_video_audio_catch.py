...
class m3u8Download():
    ...
    def get_urls(self,tag = 'video' ):
        NO_INIT = True
        if tag == 'video':
            self.m3u8_path = self.m3u8_video_path
        else:
            self.m3u8_path = self.m3u8_audio_path
        self.download_path = self.down_path+'_' + tag + '/'
    ...
    def toMp4(self,video_path = '',audio_path = ''):
        # print(video_path)
        if len(video_path) and len(audio_path):
            os.system("ffmpeg -i " +video_path+ ' -i '+ audio_path+" -codec copy " + self.combine_path + "out.mp4")
            return
        if len(video_path):
            os.system("ffmpeg -i " + video_path + " -codec copy " + self.combine_path + "out.mp4")
            return
        if len(audio_path):
            os.system("ffmpeg -i " + audio_path + " -codec copy " + self.combine_path + "out.mp4")
            return

        print("视频 {}.mp4 转换成功".format(self.video_name))
    # 将所有下载好的分片文件组合成一个文件
    ...
    def combine(self,file_name):
        ...
        # print(file_list)
        file_path = self.combine_path +file_name+ self.fileEnd
...
if __name__ == '__main__':
    ...
    for tag in Tag:
        ...
        m3u8.mutiDownload()
        m3u8.file_walker()
        files.append(m3u8.combine(tag))
    m3u8.toMp4(files[0],files[1])
